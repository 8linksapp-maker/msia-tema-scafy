...
export default {
  ...
  // Safelist — classes geradas dinamicamente via interpolação (`bg-cat-${c}`,
  // `btn-${variant}`) que o JIT não detecta no scan. Forçamos a inclusão no build.
  safelist: [
    // botões do tema (WhatsappCta monta `btn-${variant}` → literal não aparece no source)
    'btn-whatsapp',
    'btn-primary',
    'btn-ghost',
    'bg-cat-terracota',
